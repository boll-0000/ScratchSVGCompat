...
Windows環境では、GTK が必要になる場合が多くあります（CairoSVG などのライブラリが利用するため）。

GTK のインストールが面倒な場合は、`*.py` 版ではなく `*.exe` 版をお試しください。

`*.exe` 版は `*.py` 版と同じソースコードからビルドされていますが、GTK を実行ファイル内に含めているため、別途 GTK をインストールする必要はありません。

### 表示結果について

アプリ内部で表示される結果と、実際に Chrome などのブラウザで開いた結果が異なる場合があります。

これはレンダラーの違いによるものであり、必ずしもどちらかが正しいというわけではありません。ご了承ください。
...
For reasons that are still unknown, using Inkscape may occasionally produce less accurate output than not using it.

### About the Python version

On Windows, GTK is often required because it is used by libraries such as CairoSVG.

If you do not want to install GTK separately, try the `*.exe` version instead of the `*.py` version.

The `*.exe` version is built from exactly the same source code as the `*.py` version, but GTK is bundled inside the executable, so no separate GTK installation is required.

### About the preview

The preview shown inside the application may differ from the result displayed in Chrome or other web browsers.

This is caused by differences between rendering engines and does not necessarily mean either result is incorrect.
